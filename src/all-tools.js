...
const executeCommandTool = tool(
    async ({command, workingDirectory, longRunning = false}) => {
        const cwd = workingDirectory || process.cwd();
        console.log(`  [工具调用] execute_command("${command}")${workingDirectory ? ` - 工作目录: ${workingDirectory}` : ''}${longRunning ? ' - 长期运行' : ''}`);
        ...
        return new Promise((resolve, reject) => {
            if (longRunning) {
                ...
                child.unref();
                console.log(`  [工具调用] execute_command("${command}") - 已在后台启动，pid: ${child.pid}`);
                resolve(`长期运行命令已在后台启动: ${command}${workingDirectory ? `\n工作目录: ${workingDirectory}` : ''}\npid: ${child.pid}`);
                return;
            }
    ...
    },
    {
        ...
        schema: z.object({
            ...
            longRunning: z.boolean().optional().describe('是否为长期运行命令，例如启动开发服务器。为 true 时会在后台启动并立即返回'),
        }),
    }
);
